...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        body{font-family:'Sora',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;font-size:17px;
        ...
        .btn{display:inline-flex;align-items:center;gap:.4rem;padding:.42rem 1rem;border-radius:50px;
             font-size:.8rem;font-weight:600;font-family:'Sora',sans-serif;cursor:pointer;border:none;
...
// BUG FIX: proper separate className assignment (not concatenation)
...
async function loadDateSelector(){
    ...
    try{
        ...
        if(data.dates&&data.dates.length){
            ...
            // BUG FIX: await both sequentially — analytics race condition removed
            await loadDashboard();
            await loadAnalytics();
...
function setDateRange(from,to){document.getElementById('fromDate').value=from;document.getElementById('toDate').value=to;loadAnalytics();}
...
document.getElementById('exportPdfBtn').addEventListener('click',()=>{
    ...
    if(expenses.length){
        ...
        doc.autoTable({head:[['Category','Description','Amount (₱)']],body:expenses.map(e=>[e.category,e.description,peso(e.amount)]),
            startY:y,styles:{fontSize:8},headStyles:{fillColor:[167,139,250],textColor:255,fontStyle:'bold'},margin:{left:14,right:14}});
        y=doc.lastAutoTable.finalY+4;
    }
    const ts=rows.reduce((s,r)=>s+r.sold_peso,0);
    const te=expenses.reduce((s,e)=>s+parseFloat(e.amount||0),0);
    doc.setFontSize(9);doc.setTextColor(40,40,40);
    doc.text(`Total Sales: ${peso(ts)}   Total Expenses: ${peso(te)}   Est. Net: ${peso(ts-te)}`,14,y+2);
...
// BUG FIX: Logout uses modal() helper properly
