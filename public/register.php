...
<head>
    ...
    <style>
        ...
        [data-theme="light"] .field select { background: var(--surface-2); }
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        #themeToggle {
            ...
            font-family:'Sora',sans-serif; backdrop-filter:blur(8px); transition:.18s;
        }
        ...
        .card {
            ...
            box-shadow:
                ...
                0 32px 80px -16px rgba(0,0,0,.75),
                0 0 120px -40px var(--teal-glow);
        ...
        .brand-logo {
            width:65px; height:65px; margin:0 auto .9rem;
            background:linear-gradient(135deg, var(--teal), var(--teal-dark));
            border-radius:20px; display:flex; align-items:center; justify-content:center;
            font-size:1.65rem;
            box-shadow:0 8px 40px var(--teal-glow), 0 0 0 1px rgba(41,182,200,.2);
            transition:transform .2s, box-shadow .2s;
        }
        .brand-logo:hover { transform:translateY(-2px) rotate(-4deg); box-shadow:0 14px 50px var(--teal-glow); }
        .brand-name { font-size:1.65rem; font-weight:700; letter-spacing:-.04em; color:var(--text); }
        .brand-sub  { font-size:.72rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:.12em; margin-top:.35rem; }
        ...
        .field label {
            ...
            letter-spacing:.1em; color:var(--text-muted); margin-bottom:.5rem;
        }
        .field input, .field select {
            width:100%; background:var(--surface-2); border:1px solid var(--border);
            border-radius:13px; color:var(--text); font-family:'Sora',sans-serif;
            font-size:.95rem; padding:.9rem 1rem; outline:none;
        ...
        .pw-toggle {
            ...
            padding:.2rem; font-size:.9rem; transition:color .15s;
        }
        ...
        .strength-label { font-size:.65rem; color:var(--text-muted); margin-top:.35rem; }

        /* Button */
        .btn-register {
            width:100%; margin-top:.75rem;
            background:linear-gradient(135deg, var(--teal), var(--teal-dark));
            color:#05111e; border:none; padding:1rem; border-radius:50px;
        ...
        .btn-register:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 10px 36px rgba(41,182,200,.55); }
        ...
        .alert {
            ...
            margin-top:.9rem; font-size:.85rem; font-weight:500; line-height:1.5;
        }
...
<!-- BUG FIX: theme.js at end of body so DOM is ready before script runs
     (was in <head>, button not yet in DOM when applyTheme() tries to find #themeToggle) -->
...
document.getElementById('registerBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        try { data = JSON.parse(text); } catch {
            showError('Server error. Make sure XAMPP is running and you\'re accessing via http://localhost/…');
